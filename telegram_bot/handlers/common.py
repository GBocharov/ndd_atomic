...
async def process_rubrics(msg: Message, state: FSMContext):
    ...
    await msg.answer("2 ШАГ. Введите 2 даты диапазона поиска или нажмите пропустить. Формат: 2 даты через пробел без кавычек: '20000101 20100101': ",
...
async def handle_skip_button(callback: types.CallbackQuery, state: FSMContext):
    await state.update_data(dates = '20100101 20200101')
    user_data = await state.get_data()
    builder = InlineKeyboardBuilder()
    builder.add(types.InlineKeyboardButton(
        text="Отменить ❌",
